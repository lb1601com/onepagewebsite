...
(
	...
	(
		...
		(
			...
			'exclude'                 => true,
			'inputType'               => 'select',
			//added taxonomy by andreas.schempp
			'options'                 => array('text', 'alias', 'longtext', 'number', 'decimal', 'date', 'checkbox', 'select', 'tags', 'url', 'file', 'taxonomy'),
		...
		(
			...
			'inputType'               => 'tableTree',
			'eval'                    => array('fieldType'=>'checkbox', 'title' => &$GLOBALS['TL_LANG']['tl_catalog_fields']['itemsTitle']),
			'load_callback'           => array(
		...
		'imageWidth' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_catalog_fields']['imageWidth'],
			'inputType'               => 'text',
		...
		'imageHeight' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_catalog_fields']['imageHeight'],
			'inputType'               => 'text',
			'eval'                    => array('rgxp' => 'digit')
		),
...
//added by thyon
		...
		'taxonomyRoot'          => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_catalog_fields']['taxonomyRoot'],
			'inputType' 							=> 'tableTree',
			'eval'      							=> array('mandatory' => true, 'fieldType' => 'radio', 'tableColumn'=> 'tl_taxonomy.name'),
		),        
...
class tl_catalog_fields extends Backend
{
	...
	public function loadCatalogFields(DataContainer $dc)
	{
		...
		if (!strlen($act) && $dc->table == 'tl_catalog_fields' && $dc->id)
		{
			// regenerate dca
			$this->Catalog->regenerateDca($dc->id);
		}
	}
	...
	public function renderField($arrRow)
	{
		...
		$types = array
			(
				...
				'url' 			=> 'page.gif',
				'file' 			=> 'iconPLAIN.gif',
				'taxonomy' 	=> 'system/modules/taxonomy/html/icon.gif',
			);
